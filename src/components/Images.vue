  ...
  <div>
    ...
    <section>
      <div class="img_container" v-for="(img, index) in images" v-bind:key='index'  data-test="img_container">
        ...
        <div class="img_title_section">                                    <!-- APUNTES -->
          <h2>{{img.title}}</h2> 
        </div>
        ...
        <!--
        <picture v-on:click="removeImage(index)">                         
          <source media="(min-width:768px)" v-bind:srcset="img.url">      
          <img v-bind:src="img.thumbnailUrl" alt="Image" style="width:100%;">
        </picture>
        --> 
